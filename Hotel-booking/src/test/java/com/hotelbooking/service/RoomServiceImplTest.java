...
class RoomServiceImplTest {
    ...
    void searchRooms_normalizesInput() {
        ...
        filter.setCity("  goa  ");
        when(roomRepository.search(any(), any(), any(), any(), any(), any(Pageable.class))).thenReturn(Page.empty());

        roomService.searchRooms(filter, Pageable.unpaged());

        verify(roomRepository).search(isNull(), eq("goa"), isNull(), isNull(), isNull(), eq(Pageable.unpaged()));
    }
